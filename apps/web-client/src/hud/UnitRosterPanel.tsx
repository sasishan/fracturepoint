...
export function UnitRosterPanel(): React.ReactElement | null {
  ...
  const handleUnitClick = (unit: LocalUnit) => {
    ...
    cameraService.focusOnIdZoom(unit.provinceId);
  };
      ...
      {!collapsed && (
        <>
          ...
          <div style={{ overflowY: 'auto', flex: 1 }}>
            ...
            {section === 'buildings' && (() => {
              if (totalBuildings === 0) {
                return <div style={{ padding: '12px 10px', color: '#3a4a5a', fontSize: 15, letterSpacing: 1 }}>NO BUILDINGS CONSTRUCTED</div>;
              }
              // Group by building type → list of provinces
              const byType = new Map<string, number[]>();
              for (const prov of builtProvinces) {
                const bset = allBuildings.get(prov.id) ?? new Set();
                for (const bt of bset) {
                  if (!byType.has(bt)) byType.set(bt, []);
                  byType.get(bt)!.push(prov.id);
                }
              }
              const sortedTypes = [...byType.keys()].sort((a, b) =>
                (BUILDING_DEF[a as keyof typeof BUILDING_DEF]?.label ?? a)
                  .localeCompare(BUILDING_DEF[b as keyof typeof BUILDING_DEF]?.label ?? b)
              );
              return sortedTypes.map(bt => {
                const def    = BUILDING_DEF[bt as keyof typeof BUILDING_DEF];
                const col    = BUILDING_DOMAIN_COLOR[def.domain];
                const provIds = byType.get(bt)!;
                return (
                  <div key={bt} style={{ borderBottom: '1px solid rgba(30,45,69,0.3)' }}>
                    ...
                    <div style={{
                      ...
                      <img src={`/assets/buildings/${BUILDING_PNG_FILE[bt as keyof typeof BUILDING_PNG_FILE]}`} alt={def.label} style={{ width: 12, height: 12, objectFit: 'contain', opacity: 0.9 }} onError={(e) => { (e.currentTarget as HTMLImageElement).style.display = 'none'; }} />
                      <span style={{ color: col, fontSize: 14, letterSpacing: 1.5, fontWeight: 700, flex: 1 }}>
                        {def.label.toUpperCase()}
                      </span>
                      <span style={{ color: '#7d8fa0', fontSize: 13 }}>{provIds.length}</span>
                    </div>
                    {/* Province locations */}
                    <div style={{ padding: '3px 10px 5px' }}>
                      {provIds.map(pid => {
                        const p = provinces.find(pr => pr.id === pid);
                        const name = p ? (p.city || p.country) : `Zone ${pid}`;
                        return (
                          <button key={pid} onClick={() => { cameraService.focusOnIdZoom(pid); cameraService.selectBuilding(pid, bt); AudioManager.playRandom(...VOICE.selectBuilding); }} style={{
                            display: 'block', width: '100%', background: 'transparent',
                            border: 'none', cursor: 'pointer', textAlign: 'left',
                            padding: '2px 0', fontFamily: 'Rajdhani, sans-serif',
                            color: '#7d8fa0', fontSize: 14, letterSpacing: 0.5,
                          }}>
                            📍 {name.toUpperCase()}
                          </button>
                        );
